...
node("nodejs"){
    ...
    stage('Clone Git Repo') {
        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
            git branch: 'main', credentialsId: 'github-goitProjects', url: 'git@github.com:goitProjects/SBC_backend.git'
        }
    }
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-SBC-Backend/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-SBC-Backend/src/gcp-config/keys.json"
                sh "cp /root/keys/sbc-backend/.env /root/slave_jenkins/workspace/Build-SBC-Backend/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  sbc-backend.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/sbc-backend"
            }
        }
    }
